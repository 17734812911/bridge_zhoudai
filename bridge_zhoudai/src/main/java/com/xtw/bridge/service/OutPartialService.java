...
public class OutPartialService implements OutPartialDao{
    ...
    @Override
    public List<HashMap<String,String>> queryOutPartitionTrend(int partitionId, String terminalId) {
        // 根据分区ID和设备ID查询该分区的外置局放设备的历史趋势(最大放电量、对应时间)
        return outPartialDao.queryOutPartitionTrend(partitionId, terminalId);
    }

    @Override
    public List<HashMap<String, String>> historicalAnalysis(int partitionId, String terminalId) {
        // 根据分区ID和设备ID查询该分区的外置局放历史分析(最大放电量、最大放电频次、时间)
        return outPartialDao.historicalAnalysis(partitionId, terminalId);
    }

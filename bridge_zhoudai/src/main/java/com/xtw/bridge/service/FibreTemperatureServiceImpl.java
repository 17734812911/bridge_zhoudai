...
public class FibreTemperatureServiceImpl implements FibreTemperatureService {
    ...
    public int updateDataTime() {
        int result = 0;
        try{
            result = fibreTemperatureDao.updateDataTime(sdf.parse(sdf.format(new Date())));
        } catch (Exception e){
     ...
     * 调用分页插件完成分页
     */
    private PageInfo<FibreTemperature> getPageInfo(PageRequest pageRequest, Integer partitionId, Date beginTime, Date endTime) {
        List list = new ArrayList();
        int pageNum = pageRequest.getPageNum();
        int pageSize = pageRequest.getPageSize();
        PageHelper.startPage(pageNum, pageSize);
        List<FibreTemperature> fibreTemperatureList = fibreTemperatureDao.queryDatasPage(pageRequest, partitionId, beginTime, endTime);
        for (FibreTemperature  fibreTemperature: fibreTemperatureList) {
            LinkedHashMap<String, Object> linkedHashMap = new LinkedHashMap<>();
            String[] fibreTemperatureDatas =  fibreTemperature.getDatas().split(",");
            double[] doubleArray = MyUtils.toDoubleArray(fibreTemperatureDatas);
        ...
        return new PageInfo<FibreTemperature>(list);
    }
    ...
    public List<FibreTemperature> queryAllPartitionMaxValue() {
        // 获取所有分区三相的最新值
        return fibreTemperatureDao.queryAllPartitionMaxValue();
    }
